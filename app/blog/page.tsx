...
const POSTS = [
  ...
  { slug: "how-we-built-mvp-in-12-days", category: "Case Study", title: "How We Built a Client's MVP in 12 Days — Behind the Scenes", excerpt: "A full breakdown of how we took a startup from zero to a live product in under two weeks — what we prioritized and what we cut.", readTime: "10 min", date: "Feb 12, 2026", featured: false, gradient: "linear-gradient(135deg, #080808, #0f2027)" },
  { slug: "nextjs-vs-react-2026", category: "Development", title: "Next.js vs React — Which Should You Build With in 2026?", excerpt: "Both are powerful. Both are popular. But for most client projects and SaaS products, one is clearly the better choice.", readTime: "9 min", date: "Jan 30, 2026", featured: false, gradient: "linear-gradient(135deg, #0d1a0d, #1a3a1a)" },
];
...
export default function Blog() {
  ...
  const filteredRest = activeFilter === "All" ? rest : rest.filter((p) => p.category === activeFilter);
    ...
    <>
      ...
      <main>
        <PageHero
          label="Insights"
          title="Ideas, Guides &"
          titleAccent="Industry Thinking."
          sub="Practical content for founders, product teams, and business owners building in the digital age."
        />

        {/* Featured */}
        <section className="px-6 lg:px-12 pb-16 max-w-7xl mx-auto">
          <div className="section-label mb-6 reveal">Featured Article</div>
          <a href={`/blog/${featured.slug}`} className="grid lg:grid-cols-2 overflow-hidden rounded-2xl border border-white/[0.07] hover-lift cursor-pointer reveal block">
            <div className="h-64 lg:h-auto flex items-center justify-center" style={{ background: featured.gradient }}>
            ...
            <div className="bg-ink p-10 flex flex-col gap-4">
              ...
              <h2 className="font-display text-[clamp(22px,2.5vw,30px)] font-medium text-snow tracking-[-0.8px] leading-[1.2]">{featured.title}</h2>
        ...
        <section className="px-6 lg:px-12 pb-24 max-w-7xl mx-auto">
            ...
            <div className="flex flex-wrap gap-2">
              {CATS.map((c) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-full border text-[12px] font-light transition-all ${
                    activeFilter === c
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredRest.map((post) => (
              <a key={post.slug} href={`/blog/${post.slug}`} className="group bg-carbon border border-white/[0.07] rounded-2xl overflow-hidden hover-lift reveal flex flex-col">
                <div className="h-44 flex items-center justify-center" style={{ background: post.gradient }}>
