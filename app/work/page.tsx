...
function MockupVisual() {
  return (
    <div className="w-[68%] bg-white/[0.05] backdrop-blur-sm border border-white/[0.1] rounded-2xl p-5">
      ...
      {[75, 50, 90, 35, 65].map((w, idx) => (
        <div key={idx} className="h-2 rounded-full mb-2.5 last:mb-0" style={{ width: `${w}%`, background: "rgba(255,255,255,0.07)" }} />
      ))}
    </div>
  );
}
      ...
      <main>
        ...
        <section className="px-6 lg:px-12 pb-32 max-w-7xl mx-auto">
          <div className="grid md:grid-cols-2 gap-6">
            {filtered.map((p) => (
              <div key={p.title} className="group rounded-2xl overflow-hidden border border-white/[0.07] hover-lift cursor-pointer reveal">
                {/* Visual */}
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex flex-wrap gap-1.5 mb-3">
                        {p.tags.map((t) => (
                          <span key={t} className="px-2.5 py-1 rounded-full border border-amber/20 bg-amber/[0.06] font-mono text-[10px] text-amber/80">
                            {t}
                          </span>
                        ))}
                      </div>
